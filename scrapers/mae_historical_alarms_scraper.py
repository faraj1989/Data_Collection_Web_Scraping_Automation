...
Unlike a full page reload, this keeps the same browser session/page open the
whole time and just re-clicks Export every MAE_HISTORICAL_INTERVAL_SECONDS
(default 5 minutes) so newly occurred/cleared alarms keep showing up in the
"last 7 days" view without paying the cost of a fresh login + navigation each
cycle. Only re-logs in if the session actually drops.
"""
import os
import sys
import shutil
...
def main():
    try:
        ...
        print(f"Ready. Exporting every {INTERVAL_SECONDS} seconds (session stays open).")
        while True:
            try:
                ...
                driver.switch_to.default_content()
                session_dropped = "login" in driver.current_url.lower() or len(
                ...
                if session_dropped:
                    ...
                    login()
                if not find_frame_with_export():
                    print("Could not recover the Export frame. Exiting.")
                    break
